...
python "$env:CLAUDE_PLUGIN_ROOT/skills/build-metadata/scripts/build-metadata.py" <edcType> <excelPath>
```
...
详见 `$CLAUDE_PLUGIN_ROOT/skills/build-metadata/reference/edc-sheet-mapping.md`——仅在解析异常排查或新增 EDC 类型时需要。
...
见 `$CLAUDE_PLUGIN_ROOT/skills/build-metadata/reference/edc-sheet-mapping.md`「hasOther 标记规则」节。

---

## 新增 EDC 类型（扩展路径）

新增 EDC 类型需同步修改的文件清单，见 `$CLAUDE_PLUGIN_ROOT/skills/build-metadata/reference/edc-sheet-mapping.md` 末节。
